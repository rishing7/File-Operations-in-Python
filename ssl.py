...
def sns_Alert(dName, eDays, sslStatus):
    if dName == 'fota.mst.kai.jiophone.net':
      sslStat = dName + ' SSL certificate will be expired in ' + eDays +' days!! '
      snsSub = dName + ' SSL Certificate Expiry ' + sslStatus + ' alert'
      print(sslStat)
      print(snsSub)
      response = client.publish(
      TargetArn="arn:aws:sns:us-west-2:984150946756:SSL-CHECK",
      Message= sslStat,
      Subject= snsSub
      )
    elif dName == 'fota.mst.kai.jiophone.net':
      sslStat = dName + ' SSL certificate will be expired in ' + eDays +' days!! '
      snsSub = dName + ' SSL Certificate Expiry ' + sslStatus + ' alert'
      print(sslStat)
      print(snsSub)
...
def lambda_handler(event, context):
    f = ['fota.mst.kai.jiophone.net','fota.mst.sit.jiophone.net']
    for dName in f:
        print(dName)
